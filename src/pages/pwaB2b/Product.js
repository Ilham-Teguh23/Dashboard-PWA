...
const Product = () => {
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                await axios({
                    url: `${process.env.REACT_APP_API_PWA_TNOS_DSBRD_URL}/pwa-revamp/product/${id}`,
                    method: "DELETE"
                }).then((response) => {
    ...
    const TableRow = ({ num, sections, column, harga, harga_dasar, include_tnos_fee, include_ppn, tnos_fee, platform_fee, id, setting }) => (
        <tr>
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_API_PWA_TNOS_DSBRD_URL}/pwa-revamp/product/${section_id}`);
            if (!response.ok) {
            ...
            let extractedData = Array.isArray(data.data.product) ? data.data.product : Array.isArray(data) ? data : [];
                            ...
                            <Col md={5}>
                                <Form>
                                    <Form.Group id="topbarSearch">
                                        <Form.Label>Cari Nama Product</Form.Label>
