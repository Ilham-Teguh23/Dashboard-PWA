...
export default () => {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch(`https://api-tab.tnos.app/api/organization/account/user/all`);

            const data = await response.json()
            let extractedData = Array.isArray(data.data) ? data.data : Array.isArray(data) ? data : []

            let filteredData = extractedData.filter(item => item.fcmtoken !== null);
    ...
    useEffect(() => {
        if (selectedUsers.length > 0) {
            localStorage.setItem('selectedUsersB2B', JSON.stringify(selectedUsers))
        } else {
            localStorage.removeItem('selectedUsersB2B')
        }
    }, [selectedUsers]);

    useEffect(() => {
        const savedSelectedUsers = localStorage.getItem('selectedUsersB2B');
    ...
    const handleSelectAll = () => {
        ...
        if (!selectAll) {
            setSelectedUsers(user.map((item) => ({ username: item.username, fcmtoken: item.fcmtoken })))
        } else {
            setSelectedUsers([])
        }
    }
    ...
    const handleCheckboxChange = (userId, fcmToken) => {
        const newSelectedUsers = selectedUsers.find(user => user.username === userId)
            ? selectedUsers.filter(user => user.username !== userId) 
            : [...selectedUsers, { username: userId, fcmtoken: fcmToken }]
    ...
    const handleSubmit = async () => {
        try {
            const storeUsers = localStorage.getItem("selectedUsersB2B")
            const selectedUsers = JSON.parse(storeUsers)

            const memberCodes = selectedUsers.map((user) => user.username)
                                                                    ...
                                                                    <tr key={index}>
                                                                        <td className="text-center">
                                                                            <input
                                                                                type="checkbox"
                                                                                style={{ cursor: 'pointer' }}
                                                                                checked={selectedUsers.some(user => user.username === item.username)}
                                                                                onChange={() => handleCheckboxChange(item.username, item.fcmtoken)}
                                                                            />
                                                                        </td>
                                                                        <td>{item.nama}</td>
                                                                        <td className="text-center">{item.country_code}{item.phone_number}</td>
                                                                    </tr>
                                                                )
                                                            })
                                                        ) : (
